...
const PurchasedRequestDetail: React.FC = () => {
    ...
    const [selectedRow, setSelectedRow] = useState<PurchasedRequest_type | null>(); // Correct typing for state
    ...
    const { data: purchasedReq, isError, error, isLoading, isSuccess } =
        ...
        ? purchasedReq.map((item, index) =>
            createData(
                ...
                (item.items || [])
                    ...
                    .join(", "),
                (item.items || [])
                    .map((i) => (i.products || []).map((p) => p?.subcategory?.name || "").join(", "))
                    .join(", "),
                (item.items || [])
                    .map((i) => (i.products || []).map((p) => p?.category?.name || "").join(", "))
                    .join(", "),
                item?.totalPrice || 0,
