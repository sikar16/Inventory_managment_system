...
const WareHouseTable: React.FC<StoreProps> = ({ storeList }) => {
    ...
    const [storeProducts, setStoreProducts] = React.useState<{ [key: string]: any[] }>({});
    ...
    return (
        <div className="flex flex-col mx-[7%]">
                                            ...
                                            <div className='flex gap-4'>
                                                ...
                                                >
                                                    <div className="mr-5 flex h-10 w-10 items-center justify-center rounded-lg bg-primary/5 text-primary dark:bg-white/5">
                                                        {openFaqIndex === index ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                                                    </div>
                                                </button>
                                                <button className='text-[#002a47] px-2 py-1 rounded-lg' onClick={handleOpenDialog}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" width={15} height={15} viewBox="0 0 14 14" >
                                                        <path fill="#002a47" fillRule="evenodd" d="M8 1a1 1 0 0 0-2 0v5H1a1 1 0 0 0 0 2h5v5a1 1 0 1 0 2 0V8h5a1 1 0 1 0 0-2H8z" clipRule="evenodd"></path>
                                                    </svg>
                                                </button>
                                            </div>
                                        </TableCell>
                                            ...
                                            <TableCell colSpan={columns.length + 1}>
                                                <div className="pl-[62px] bg-gray-50 p-4 rounded-lg">
                                                    <div className="grid grid-cols-2 gap-4">
                                                        <p className="text-md">
                                                            Store Name: <span className="text-sm">{storeList[index].name}</span>
                                                        </p>
                                                        <p className="text-md">Products:</p>
                                                        <div className="col-span-2 grid grid-cols-2 text-sm gap-2">
                                                            {storeProducts[storeList[index].name]?.map((product, i) => (
                                                                <div key={i}>
            ...
            <Dialog open={openDialog} onClose={handleCloseDialog}>
                ...
                <DialogContent>
                    <AddProduct onAddProduct={handleAddProduct} handleCloseDialog={handleCloseDialog} />
                </DialogContent>
            </Dialog>
        </div>
    );
