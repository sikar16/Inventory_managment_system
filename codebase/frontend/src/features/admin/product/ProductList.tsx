...
export default function ProductList() {
  ...
  const filteredProducts = products?.filter((product: ProductType) => {
    ...
    const matchesTemplate =
      selectedTemplate === "" ||
      product.templateId === parseInt(`${selectedTemplate}`); // Assuming products have a templateId
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (productCategories) {
                const selectedCategoryObj = productCategories.find(
          ...
          {selectedProduct && (
            <div className="grid grid-cols-2 gap-4">
              ...
              <p className="text-md col-span-2">
                Template:
                <span className="text-sm">
                  {Array.isArray(selectedProduct?.templateAttributeType) &&
                  selectedProduct.templateAttributeType.length > 0
                    ? selectedProduct.templateAttributeType
                        .map((k) => k.name)
                        .join(", ")
                    : "N/A"}
                </span>
              ...
              {/* Attributes Section */}
              <p className="text-md col-span-2 font-semibold">Attributes:</p>
              <div className="grid grid-cols-2 gap-4 mt-2 col-span-2">
                {selectedProduct.productAttributes &&
                selectedProduct.productAttributes.length > 0 ? (
