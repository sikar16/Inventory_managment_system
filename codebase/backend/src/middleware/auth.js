...
export const isDH=(req,res,next)=>{
 const dh = req.user;
  if (dh && dh.role !== CompanyRole.DEPARTMENT_HEAD) {
    return res.status(401).json({ success: false, message: 'User not department head' });
  }
  next();
}
...
export const isLS=(req,res,next)=>{
 const ls = req.user;
  if (ls && ls.role !== CompanyRole.LOGESTIC_SUPERVISER) {
    return res.status(401).json({ success: false, message: 'User not logestistic superviser' });
  }
  next();
}
export const isFinance=(req,res,next)=>{
 const finance = req.user;
  if (finance && finance.role !== CompanyRole.FINANCE) {
    return res.status(401).json({ success: false, message: 'User not finance' });
  }
  next();
}
export const isGM=(req,res,next)=>{
 const gm = req.user;
  if (gm && gm.role !== CompanyRole.GENERAL_MANAGER) {
    return res.status(401).json({ success: false, message: 'User not general manager' });
  }
  next();
}
export const isStoreKeeper=(req,res,next)=>{
 const sk = req.user;
  if (sk && sk.role !== CompanyRole.STORE_KEEPER) {
    return res.status(401).json({ success: false, message: 'User not store keeper' });
  }
  next();
}